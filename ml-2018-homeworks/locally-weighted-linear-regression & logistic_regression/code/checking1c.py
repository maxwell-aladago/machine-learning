...
def checking1c():
    check_data = spio.loadmat('q1c_checker.mat', squeeze_me=True)
    ...
    try:
        ...
        output_size7 = check_data['output_size7'][0]
        ...
        error = q1_cross_validation_error(X5,Y3,tau5, N);
        if np.linalg.norm(np.shape(err) - output_size7):
            print('q1_cross_validation_error, WRONG OUTPUT:', np.shape(err), ', expected:', output_size7, '\n')
            c += 1
        else:
            print("q1_cross_validation_error successfully passed output size test! \n")

    except:
        c += 1
        print("cannot execute one of the functions required for q1c\n")

    if c != 0:
        print("Exiting q1c due to error\n")
        sys.exit()
